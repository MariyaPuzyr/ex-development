...
function admin_menu_theme_mobile_color_scheme(){
	...
	add_submenu_page("themes.php", __('Color scheme (mobile version)','pntheme'), __('Color scheme (mobile version)','pntheme'), 'administrator', "pn_theme_mobile_color_scheme", array($plugin, 'admin_temp'));
}
...
function def_pn_adminpage_content_pn_theme_mobile_color_scheme(){
	...
	$options['mobile_color_scheme'] = array(
		...
		'default' => $plugin->get_option('mobile_color_scheme'),
...
global $wpdb;	
	...
	$plugin->update_option('mobile_color_scheme','', $color_scheme);
...
if(function_exists('is_mobile') and is_mobile()){
	...
	function mobile_color_scheme_body_class($classes){
		...
		$classes[] = $plugin->get_option('mobile_color_scheme');
